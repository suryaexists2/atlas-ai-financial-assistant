...
_TOOL_SPECS: list[tuple[str, str, dict[str, str], tuple[str, ...]]] = [
    ...
    (
        ...
        'Earnings for a ticker.',
        {'symbol': 'string'},
        ('symbol',),
    ),
    (
        'create_price_alert',
        'Alert when a stock moves X% (operator abs|gte|lte).',
    ...
    (
        ...
        "Schedule morning briefing; time '08:00', scope watchlist|interests|both.",
        {'time': 'string', 'scope': 'string'},
        (),
    ),
    (
        'create_reminder',
        "Schedule a reminder; time '09:00', text, once=true.",
    ...
    (
        'search_emails',
        "Search Gmail; returns matching messages.",
    ...
    (
        'schedule_meeting',
        "Create a calendar event: title, natural time or ISO, attendees.",
        {'summary': 'string', 'when': 'string', 'duration_min': 'integer', 'description': 'string', 'attendees': 'strlist'},
        ('summary', 'when'),
    ),
    (
        'read_drive_doc',
        'Search and read Drive file.',
        {'query': 'string'},
        ('query',),
    ),
]
